...
#emotion_model_path =  "models/Model-baseline.hdf5"
#emotion_model_path =  "models/Model2-convs2.hdf5"
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        for b in dets:
            ...
            #roi = RGB[fY:fH, fX:fW]
            roi_ori = img_raw[fY:fH, fX:fW]
            #roi_s = cv2.resize(roi_ori, (64, 64), interpolation=cv2.INTER_LINEAR)

            if roi_ori.shape[0]!=0 and roi_ori.shape[1]!=0:
                roi_s = cv2.resize(roi_ori, (64, 64), cv2.INTER_LINEAR)
            ...
            if label == 'Male':
                ...
                cv2.rectangle(img_raw, (fX, fY), (fW, fH), (0, 128, 0), thickness=-1)
                alpha = 0.7
                cv2.addWeighted(overlay, alpha, img_raw, 1 - alpha, 0, img_raw)
                cv2.rectangle(img_raw, (fX, fY), (fW, fH),(0, 128, 0), 1)
            else:
                overlay = img_raw.copy()    
                cv2.rectangle(img_raw, (fX, fY), (fW, fH), (100, 0, 200), thickness=-1)
                alpha = 0.7
                cv2.addWeighted(overlay, alpha, img_raw, 1 - alpha, 0, img_raw)
                cv2.rectangle(img_raw, (fX, fY), (fW, fH),(100, 0, 200), 1)
